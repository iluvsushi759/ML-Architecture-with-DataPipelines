...
USING (
    SELECT DISTINCT HOSPITAL_ID, HOSPITAL_NAME, HOSPITAL_CITY, BASE_RATING
    FROM DEV.HOSPITALS_TRANSFORMED
) AS src
ON tgt.HOSPITAL_ID = src.HOSPITAL_ID
WHEN MATCHED THEN UPDATE SET
    HOSPITAL_NAME = src.HOSPITAL_NAME,
    HOSPITAL_CITY = src.HOSPITAL_CITY,
    BASE_RATING = src.BASE_RATING
WHEN NOT MATCHED THEN INSERT (HOSPITAL_ID, HOSPITAL_NAME, HOSPITAL_CITY, BASE_RATING)
VALUES (src.HOSPITAL_ID, src.HOSPITAL_NAME, src.HOSPITAL_CITY, src.BASE_RATING);
-- Merge claims
